...
// Ce composant gère la recherche d'artistes côté frontend.
// Il propose deux variantes d'affichage : une version d'accueil et une version compacte.
export function ArtistSearchPanel({
  ...
  return (
    ...
    >
      ...
      {errorMessage ? (
        <div
          className={`rounded-[1.25rem] border border-red-400/30 bg-red-500/10 px-4 py-3 text-red-100 ${
      ...
      {!errorMessage && hasSearched && !isLoading && results.length === 0 ? (
        <div
          className={`rounded-[1.25rem] border border-white/10 bg-black/20 px-4 py-3 text-slate-300 ${
            isCompact ? "text-xs" : "text-sm"
          }`}
        >
          ...
          {results.map((artist) => {
            ...
            return (
              <button
                ...
                className={`rounded-[1.5rem] border text-left shadow-lg shadow-black/10 transition ${
                  isSelected
